...
/*
	Stockfishの探索部を参考にする時の注意
	
	Stockfish 16になって、評価値のスケールに大きな変更があった。
	そのため、やねうら王にそのまま持ってくることはできない。
	
	例えば、
		if (beta > 123)
	のような式があったとして、このスケールをやねうら王に変換するには、
	123を1.09ぐらいで割ってやる必要がある。
...
// Stockfish 14の時の式の方が良さげ。(V7.74z6 vs V7.74z12 , V7.74taya-t20 vs V7.74taya-t30)
...
void update_continuation_histories(Stack* ss, Piece pc, Square to, int bonus)
{
	//for (int i : {1, 2, 3, 4, 6})
	for (int i : {1, 2, 4, 6})
	// ⇨　3手前も更新するの、強くなってない気がする。(V7.74taya-t20 vs V7.74taya-t21)
	{
