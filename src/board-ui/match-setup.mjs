 ...
 * @param {{ fetchImpl?: typeof fetch, enemiesUrl?: string, formationsUrl?: string, difficultyUrl?: string }} [options]
 */
export async function loadMatchSetupOptions(options = {}) {
  const fetchImpl = options.fetchImpl || fetch;
  const [enemyResponse, formationResponse, difficultyResponse] = await Promise.all([
  ...
  return { enemies, formations, difficulties };
}
 ...
 * @param {{ enemyId: string, formationId: string, difficultyId: string }} selection
 */
export function buildMatchSearch({ enemyId, formationId, difficultyId }) {
