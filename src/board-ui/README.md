...
対局中の「ヒント」は敵の強さと独立して停止までAI探索を続け、現局面の最善手・次善手・3番手、
各評価値、深さ、直近の探索局面数と6手先までの読み筋をリアルタイム表示する。1,000ノード単位で
ブラウザへ制御を返すため、解析中も停止操作ができる。解析中に再度押すかタブを
非表示にすると停止し、指し手は「5二銀」のような日本語表記で残す。「待った」は
...
ヒントの短い探索反復・停止・評価値通知、上位3手とPVの日本語表示整形、待ったの手番履歴復元、
取り消し後の千日手履歴も確認する。
