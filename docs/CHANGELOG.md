...
- 名前・呼称・難易度、村移動、夜古火姫との会話、初回勝利報酬、経験値・レベル、棋貨、店、
  宝箱、サブクエスト、定跡集、復活の呪文を実装
- 各村を通常敵への勝利で次の敵・ボスを順次解禁する一本道コースへ変更。宝箱は任意分岐、
  店とクエストは下部施設とし、既存セーブの撃破・開封記録から進行を復元
- 主人公と夜古火姫の現在地、次の目的地、解禁・勝利済み状態を接続線付きコースマップへ表示
- 敵専用の戦法定跡を追加し、棒銀村、四間飛車村、右四間＋エルモ囲い村、居飛車村、
  穴熊村の敵が学習テーマに沿う戦法・囲いを完成するまで`go searchmoves`で候補手を限定
- YaneuraOu v4.73_book公式リリースの`standard_book.db`をWASM初期化前に読み込み、
  敵専用定跡がない対局では標準定跡を使用。取得・エンジン対応に失敗しても探索のみで継続
- Hostingビルドをvendor許可リスト、敵マスタから参照されるNNUEだけのコピー、標準定跡DBの
  明示コピーへ変更し、未承認バイナリを意図せず配布しない構成へ強化
- 「思考の砂時計」と「迷いの香」を対局開始時に1個消費し、敵の探索量・候補手ランクへ反映
- ヒントを敵の強さと独立した停止可能な短い探索反復へ変更し、最善手・次善手・3番手、評価値、
  深さ、直近の探索局面数、各候補6手先までの読み筋をリアルタイム更新。WASMがページ操作を止めないよう
  1,000ノードごとに制御を返し、「5二銀」のような日本語表記とタブ非表示時の自動停止にも対応
- 駒の人格・軍勢・損耗継承を主軸とした短編キャンペーンを削除し、通常の将棋対局をRPGの戦闘とする方針へ統一
- 途中実装で失われていた稽古相手とversion 1復活の呪文発行互換を維持
...
- 7つの村、夜古火姫、段階的な将棋学習、定跡書、レベル成長、追加アイテム、
  素材差し替え条件と未確定事項を整理した`GAME_DESIGN.md`を追加。名前入力、村ごとの一本道コース、
  初回勝利経験値、任意の宝箱分岐・店・棋貨・サブクエスト、主人公の短い内心、
  やこび姫の人物像、任意ボイス方針を整理
...
- 敵の直前手について、移動元を黄色、移動先を橙色で盤上に表示する視認性改善を追加
- やこび姫のミニキャラと吹き出しを対局盤へ追加し、先手・後手の駒配置から棒銀、
  金矢倉囲い、右四間飛車を検出して1局につき各1回知らせる戦形コールアウトを追加
...
### Fixed
- 本編の埋め込み対局が勝利時に撃破済みIDを先行保存すると、本編側で初勝利と認識されず、
  第1章ボスを倒しても第2章が解禁されない問題を修正。影響を受けた既存セーブも、起動時に
  撃破済みボスから解禁章を復旧する
