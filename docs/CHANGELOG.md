...
## [Unreleased] マイルストーン1: 持ち駒を打つ操作のドラッグ&ドロップ対応（2026-07-14）

### Added
- `src/board-ui/board.js`: `_drawHandPiece()`を追加し、持ち駒を駒種ごとに個別のSVG要素
  （旧実装は持ち駒全体を1つの`<text>`にまとめており、クリックすると常に`hand[0]`＝
  先頭の駒種が選択される不具合があった）として描画するよう変更
- `_handleHandPointerDown()` / `_createDragGhost()` / `_cellAtClientPoint()`を追加し、
  Pointer Events APIによるドラッグ&ドロップで持ち駒を打てるようにした。ドラッグ中は
  ポインタに追従する半透明のゴースト駒を表示し、`document.elementFromPoint()`と
  盤面要素の`data-x`/`data-y`属性（`_drawCellHitboxes()`・`_drawPieceAt()`双方に付与）から
  ドロップ先マスを特定する
- 移動量が閾値（6px）未満の場合は従来の「クリックで選択→着手先マスをクリック」方式に
  自動的にフォールバックする（マウスでもタッチでも両方の操作方法を維持するため）

### Notes
- SVGは`width:100%`かつ`viewBox`でスケーリングされるため、座標変換は`getScreenCTM().inverse()`
  で行う必要がある（clientX/Yをそのままtranslateに使うとスケール不一致でずれる）
- Playwrightで検証: (1) 持ち駒の飛車をドラッグして空きマス(5,5)にドロップ→USI形式で
  `R*5e`が生成され、持ち駒が0枚になり盤面に飛車が配置されることを確認。
  (2) ドラッグせずクリックのみ→着手先クリックの2段階操作でも同じ`R*5e`が生成されることを確認。
  検証は独立したテスト用HTML（`board.shogi.initializeFromSFENString()`で持ち駒ありの
  局面を直接構築）を用い、作業後に削除済み
