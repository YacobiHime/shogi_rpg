...
- **将棋AIの思考は必ずWeb Worker内で実行する**。メインスレッドをブロックしてはいけない
  - ⚠️ **例外・訂正（M0検証で判明、2026-07-12）**: YaneuraOu.wasm（PThreadビルド）は
    Web Worker内から呼び出すと`createObjectURL`エラーで動作しない。**メインスレッドから
    直接呼び出す方式を正式採用している**。詳細は下記「WASM将棋エンジン統合時の必須知識」を参照
